...
export const ConfirmPinSeed = () => {
  const { state } = useLocation("/enter-pin-seed");
  const { deviceKey, otp } = state || "";
  const navigate = useNavigate();
  const [OTP, setOTP] = useState([]);

 console.log(state,'oooyeee')
 console.log(otp,'0o0o0o0')
  ...
  const HamdleConfirmPin = async() => {
    if (OTP.length < 6) {
      ...
      navigate("/enter-seed", { state: { deviceKey: deviceKey, pin: OTP } });
    }
  };
